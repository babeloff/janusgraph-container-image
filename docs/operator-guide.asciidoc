...
the `JG__*` edits will be applied for each back-end.
| no default value

| `JG__*`
| Any JanusGraph configuration option to override in the template properties file,
specified with an outer `JANUS` namespace (e.g., `JG__foo_storageHostname`).
...
The `JG_TEMPLATE` environment variable is used to define a template JanusGraph properties file.
...
docker run --name janusgraph2-default \
    -e JG_TEMPLATE=cql-es \
    -e JG__default_storageCachePercentage='.storage.cache-percentage=80' \
